...
// get according to city
export const CityServiceList = (cityId) => async(dispatch) =>{
    dispatch({
        type:CITY_SERVICE_LIST_REQUEST,
        payload:cityId
    ...
    try {
        ...
        const {data} = await axios.get(`/api/service/city/${cityId}/`
       
    )
    dispatch({
        type:CITY_SERVICE_LIST_SUCCESS,
        payload:data
    })
    } catch (error) {
        dispatch({
            type:CITY_SERVICE_LIST_FAIL,
            payload:
            error.response && error.response.data.message
                ? error.response.data.message
                : error.message
